...
import { Menu, X, ArrowRight, Compass, MapPin, BookOpen, Users, Brain, Star } from 'lucide-react';
  ...
  return (
    <header
      className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${
        isScrolled
          ? 'bg-[var(--bg-secondary)]/85 backdrop-blur-lg border-b border-[var(--border-default)] py-3 shadow-sm'
          : 'bg-transparent py-5'
      }`}
    >
      <div className="max-w-7xl mx-auto px-6 flex items-center justify-between">
        ...
        <div className="hidden lg:flex items-center gap-3">
          ...
          <Link
            href="/club"
            className="relative group px-5 py-2.5 rounded-full text-sm font-semibold text-white transition-all shadow-[var(--shadow-btn-gold)] hover:scale-105 active:scale-95 bg-[var(--brand-gold)] hover:bg-[var(--brand-gold-hover)]"
          >
        ...
        <button
          ...
          className="lg:hidden p-2 text-[var(--text-secondary)] hover:text-[var(--brand-blue)] rounded-xl bg-[var(--bg-surface)] hover:bg-[var(--brand-blue-light)] transition-colors"
          aria-label="Menu"
        >
      ...
      <AnimatePresence>
        {isMobileMenuOpen && (
          <motion.div
            ...
            className="absolute top-full left-0 right-0 bg-[var(--bg-secondary)]/95 backdrop-blur-lg border-b border-[var(--border-default)] shadow-lg py-4 px-6 lg:hidden flex flex-col gap-2"
          >
            {navLinks.map((link) => {
              const Icon = link.icon;
              const isActive = pathname === link.href;
            ...
            <Link
              href="/login"
              onClick={() => setIsMobileMenuOpen(false)}
              className="w-full text-center py-3 rounded-xl border border-[var(--border-default)] text-sm font-semibold text-[var(--text-secondary)] hover:text-[var(--brand-blue)] transition-colors"
            >
              Entrar
            </Link>
            <Link
              href="/club"
              onClick={() => setIsMobileMenuOpen(false)}
              className="w-full text-center py-3 rounded-xl bg-[var(--brand-gold)] text-sm font-semibold text-white shadow-[var(--shadow-btn-gold)]"
            >
              Explorar o Brasil
            </Link>
          </motion.div>
